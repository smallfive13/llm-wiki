...
DataWorks / MaxCompute 分层表入库时，页面应落在能解释长期业务口径的位置，而不是把每一层都机械建页：

- ODS：一般不建独立 wiki 页，只作为血缘起点、source 证据和表名反查的溯源入口。只有 ODS 自身承载稳定业务语义、异常清洗规则或跨系统映射时，才考虑建页并明确说明为什么不能只引用下游定义点。
- DWD：优先作为业务定义点。字段含义、过滤条件、状态枚举、主键粒度、适用边界和代码 caveats 应重点梳理到 DWD 相关 topic / asset-mapping。
- DWB：作为明细宽表定义点。重点记录从 DWD 到宽表的拼接、去重、口径合并、字段重命名和宽表适用场景。
- DWS：只记录聚合维度、时间粒度、指标口径增量和与上游 DWD/DWB 的差异；正文必须引用上游定义点，不重复展开所有基础字段。
- ADS：只记录应用过滤、展示口径、报表/接口消费场景和与上游 DWS/DWD/DWB 的差异；正文必须引用上游定义点。

`asset-mapping` 的锚点应优先落在 DWD / DWB 这类定义层。若业务答疑必须提到 DWS / ADS，只在正文说明它们是下游衍生或展示层，并用 `related_ids` 指回上游定义点；血缘方向仍是"当前页 → 上游定义/来源页"。
